...
public class RemoteRobot {
    ...
    public byte[] captureScreen(int x, int y, int width, int height) {
        try {
            Response response = sendRequest(new Request(CAPTURE_SCREEN, x, y, width, width));
            if (response.isSuccess() && response.getResultObject() != null) {
                return Base64.getDecoder().decode(response.getResultObject().toString());
            }
        } catch (Exception e) {
    ...
    private Response sendRequest(Request request) {
        ...
        try {
            ...
            socketConnector.setHandler(new io.github.jaqat.remoterobot.client.ClientHandler(request));
